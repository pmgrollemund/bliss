...
## ----eval=TRUE,include = TRUE--------------------------------------------
  param <- list(                        # define the "param" to simulate data
                ...
                beta_shapes=c("simple"),# define the shape of the "true" coefficient function
...
## ----eval=TRUE, include = TRUE,fig.height=5,fig.width=7------------------
  ...
  plot_bliss(res_bliss$data$grids[[1]],
             res_bliss$Bliss_estimate[[1]],lwd=2,ylim=ylim)
  lines(res_bliss$data$grids[[1]],
...
## For q=1
   q <- 1
   param$ylim <- range(range(res_bliss$beta_posterior_density[[q]]$grid_beta_t),
                       c(-5,5))
   param$cols <- rev(heat.colors(100))
   image_Bliss(res_bliss$beta_posterior_density,param,q=q)
   lines(res_bliss$data$grids[[q]],res_bliss$Bliss_estimate[[q]],type="s",lwd=2)
   lines(res_bliss$data$grids[[q]],res_bliss$data$betas[[q]],col=2,lwd=2,type="s")

## ----eval=TRUE, include = TRUE,fig.height=5,fig.width=7------------------
   ylim <- range(range(res_bliss$Bliss_estimate[[q]]),
                 range(res_bliss$Smooth_estimate[[q]]))
   plot_bliss(res_bliss$data$grids[[q]],
              res_bliss$Bliss_estimate[[q]],lwd=2,ylim=ylim)
   lines(res_bliss$data$grids[[q]],
         res_bliss$Smooth_estimate[[q]],lty=2)
...
## For q=2
   ...
   image_Bliss(res_bliss$beta_posterior_density,param,q=q)
   lines(res_bliss$data$grids[[q]],res_bliss$Bliss_estimate[[q]],type="s",lwd=2)
   lines(res_bliss$data$grids[[q]],res_bliss$data$betas[[q]],col=2,lwd=2,type="l")
